    ...
    stages {
        ...
        stage('Scan Docker Image main for Vulnerabilities') {
            ...
            steps {
                 sh '''
                    ...
                    trivy image --cache-dir "$WORKSPACE/.trivy-cache" --scanners vuln --exit-code 0 --severity HIGH,MEDIUM,LOW --no-progress nodemain:v1.0
                    '''
            }
        }
        ...
        stage('Scan Docker Image dev for Vulnerabilities') {
            ...
            steps {
                 sh '''
                    mkdir -p "$WORKSPACE/.trivy-cache"
                    trivy image --cache-dir "$WORKSPACE/.trivy-cache" --scanners vuln --exit-code 0 --severity HIGH,MEDIUM,LOW --no-progress nodedev:v1.0
                    '''
            }
        }
